...
| Global Claude skills (`~/.claude/skills`) | **missing on this machine**                                                                                                 | Recover skill files from the old machine / promptmi (see §6.3). Note: promptmi analysis reports several skills live on the **Windows-side** `~/.claude/skills` at `/mnt/c/Users/<user>/.claude/skills/` — verify and symlink/copy into WSL. |
...
- **Kanban (`havdm.kanban`) is not in-repo** — it lives on OneDrive (`/mnt/c/Users/<user>/OneDrive/Documents/havdm.kanban`), so board state isn't version-controlled.
