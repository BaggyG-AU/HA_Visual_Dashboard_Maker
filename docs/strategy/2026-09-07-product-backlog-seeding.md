...
**Status:** **APPROVED — rev 3, FROZEN ON SEEDING (correction C1).** GPT-6
Astra's third scoped follow-up (`docs/reviews/b7-product-backlog-codex-review-followup3.md`,
`93f56bf`) returned `CLEAR`, meeting the owner's conditional approval of
2026-09-08 (given after the second follow-up `b3541c3`; ruling
`drawer_havdm_decisions_2c2cd5491215fa0808820168`). The board is seeded from this
revision per §6 on 2026-09-08; this line is the last edit. Later changes to
the plan are made on the board, never here (D11).
...
| S-P | Export boundary design, `docs/refresh/HA_EXPORT_BOUNDARY_DESIGN_2026-07.md:357`, `:387`; vision answer 7                                                            | The strict HA-fidelity schema driving the deploy gate (two-schema split, ratified 2026-07-21) → S01.6 (rev 1). Delivery status UNVERIFIED (rev 2; paths listed rev 3): `grep -rl fidelity src/` returns four files — `src/features/theme-manager/themeOptions.ts`, `src/components/ThemeNoEffectBadge.tsx`, `src/services/canvasKeyStripper.ts` (its header comment names the remediation plan) and `src/services/yamlConversionService.ts` (`:176`, a comment about keeping fidelity loss honest) — none a schema; a word search cannot decide delivery; the brief measures it |
...
| S-U | v0.7.4-beta.5 release notes, `docs/releases/RELEASE_NOTES_v0.7.4-beta.5.md:49`, `:92`                                                                               | "Advanced multi-row alignment (`align_content`) is intentionally deferred beyond Feature 4.5 MVP" → a named E12 candidate (rev 2, owner-placed 2026-09-08). Delivery status: `grep -rn align_content src/` returns nothing; UNVERIFIED beyond that search                                                                                                                                                                                                                                                                                                                       |
...
**Reviewed and not carried (rev 1; reframed rev 2)** — older plans read in the
sweep whose remaining items are NOT carried into this backlog **by the owner's
present choice (2026-09-08)**. No supersession is claimed: each source's own
status words (deferred, planned, future) still stand as that document records
them, and the owner may carry any item later by ruling. Listed so the population
statement is honest: `docs/product/UPDATE_PLAN.md` (January 2026 component-update
plan; no successor was established in the reviewed sources — the refresh plan's WS1 is
engineering-discipline uplift, not component updates); `docs/product/SUPPORTED_VERSIONS.md` and
...
`docs/features/ENTITY_TYPE_DASHBOARD_GENERATOR.md:178–193` (five planned and
four deferred generator ideas; AI-suggested layouts is E17; the other eight keep their source's own Planned or Deferred status and are not carried, by the owner's present choice); `docs/features/ADVANCED_YAML_EDITOR_IMPLEMENTATION.md:372–419`
(code-only editor mode "deferred to future release", plus its AST / schema /
validation / diff / snippet follow-ups — deferred there, not withdrawn; delivery
status not audited);
...
(research); `docs/releases/RELEASE_NOTES_v0.3.2-beta.1.md:513–522` (editor
follow-ups restating the same editor items); and S-R above (Q3).
...
| S01.3 | A flat-canvas (scaffold) view can become a real masonry or sections view: today the two sections conversions keep the scaffold marker, and choosing masonry on a scaffold view is a no-op, so the export still treats such views as internal                  | capability | —          | S-C. Scoped (rev 2): `convertViewToSections` (`sectionsLayout.ts:452`) and `flattenSectionsView` (`:480`) both keep `_havdm_scaffold`; `normalizeViewType` (`viewsLayout.ts:157`) maps a scaffold to `masonry`, so picking masonry changes nothing. Converting to a layout-card grid (`convertViewToLayoutCard`, `viewsLayout.ts:271`) or to another type (`setViewType`, `:287`) DOES clear the marker — those paths are not the defect. Source trace, no UI run. Order after S01.2 is a preference (§5) |
...
| Story | Plain English                                                                                                                                                                                                                                                                                                                                                                    | Class      | Depends on | Source / notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S04.1 | Each of the eleven HA-03 cards gets an honest, complete property form: today the four core cards (tile, heading, entity, statistics-graph) have only the generic fallback AND are the ones shown the panel's "not yet implemented" notice; six palette cards have partial forms; `custom:fold-entity-row` has guidance only; none of the seven palette cards is shown the notice | content    | —          | S-E, **corrected from measurement 2026-09-08, twice** (review P2, follow-up P2): `src/components/PropertiesPanel.tsx` branches at `:6138`, `:6215`, `:6275`, `:6390`, `:6446`, `:6499`, `:6532` (all since `eaa9efa`, 2025-12-26); the notice at `:6718` is guarded by a NEGATED list (`:6645`) that includes the seven palette types, so it renders only for types outside it — the four core cards among them. Rev 1 stated the reverse; the reviewer's executed predicate corrected it |
...
| S06.4 | The app's "open external link" bridge is tightened (today it opens any URL the renderer passes); the restriction — for example, only links the app itself produced — is the author's candidate, chosen at the brief | capability | —                                                                                                                                           | S-N Phase C2 says only "Tighten `shell:openExternal`" (rev 2: policy labelled a candidate); `src/main.ts:205` passes the URL straight to `shell.openExternal`             |
...
| E20  | Real Home Assistant cards rendered in an isolated, preload-less window that reaches no file bridge and no IPC — third-party card code never runs in the main app, and HAVDM's own renderers stay the never-connected fallback. A time-boxed spike whose output is a go / no-go decision; rendering only if the spike says go, only on the owner's explicit trust decision, and only after Phase C lands (S06.1 and S06.4) | S-N Phases C–E; isolation ruling (rev 2) | Not estimated          |

**Named candidates already recorded for these epics (rev 1, rev 2)** — each
still needs its brief: under **E09**, sections-container nesting (S-M, deferred
by the owner 2026-08-06 because a nested child has no address in the
`(sectionIndex, cardIndex)` model); under **E12**, advanced multi-row alignment
(`align_content`) on stacks and grids (S-U, deferred beyond Feature 4.5;
owner-placed here 2026-09-08); under **E18**, a theme editor and full theme
application (S-Q, post-1.0). E19's and E20's stories are not enumerated here.
...
7. **E09–E18** follow the refresh plan's own ruled order (D3: close out Phase 7
   ...
   both are post-1.0 by their own rulings, and E20 also waits on the whole of
   Phase C (S06.1 AND S06.4) and on the owner's trust decision (rev 2).
...
| 2026-09-08 | 2         | Repaired on the owner's per-Ref rulings on the scoped follow-up `docs/reviews/b7-product-backlog-codex-review-followup.md` (`244020a`; P1/P3/P4/P8 remainders fix now, P2 approved; record in the dispositions file Round 3): S04.1's notice claim reversed to match the negated guard at `PropertiesPanel.tsx:6645`; S01.3 scoped to the two sections conversions and the masonry no-op; "reviewed and not carried" reframed as the owner's present choice with each source's own status, the WS1 claim withdrawn; S-P delivery status UNVERIFIED with the true grep result; S-U (`align_content`) added and homed under E12; S06.4's policy labelled a candidate; E20 carries the no-third-party-code boundary, the owner's trust decision and Phase C (S06.1 and S06.4) as prerequisite, size "Not estimated" | Claude Fable 5.1 |
| 2026-09-08 | 3         | Three record-wording corrections on the owner's fix-now ruling after the second scoped follow-up `docs/reviews/b7-product-backlog-codex-review-followup2.md` (`b3541c3`, P1 SEV 3; record in the dispositions file Round 4): S-P lists the four `fidelity` paths and what each is; the generator entry keeps its source's own Planned / Deferred split; the WS1 clause claims only that no successor was established in the reviewed sources. Owner approval for seeding recorded, conditional on the third follow-up                                                                                                                                                                                                                                                                                            | Claude Fable 5.1 |
| 2026-09-08 | 3 (final) | Status line only: approved (third follow-up `93f56bf` CLEAR met the owner's condition); frozen on seeding. No other change; this is the last edit                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Claude Fable 5.1 |
