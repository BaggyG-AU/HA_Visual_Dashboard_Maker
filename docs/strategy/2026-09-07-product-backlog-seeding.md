...
**Status:** Draft — rev 2 (2026-09-08), repaired on the owner's per-Ref rulings after
GPT-6 Astra's scoped follow-up (`244020a`); awaiting the same reviewer's second
scoped follow-up (OA §3.4)
...
| S-P | Export boundary design, `docs/refresh/HA_EXPORT_BOUNDARY_DESIGN_2026-07.md:357`, `:387`; vision answer 7                                                            | The strict HA-fidelity schema driving the deploy gate (two-schema split, ratified 2026-07-21) → S01.6 (rev 1). Delivery status UNVERIFIED (rev 2): `grep -rl fidelity src/` returns two theme files and two comments naming the remediation plan (`canvasKeyStripper.ts:2`, `yamlConversionService.ts:176`), none a schema — a word search cannot decide delivery; the brief measures it |
...
plan; no later document is its successor — the refresh plan's WS1 is
...
four deferred generator ideas; AI-suggested layouts is E17; the rest stay
deferred as that document says); `docs/features/ADVANCED_YAML_EDITOR_IMPLEMENTATION.md:372–419`
