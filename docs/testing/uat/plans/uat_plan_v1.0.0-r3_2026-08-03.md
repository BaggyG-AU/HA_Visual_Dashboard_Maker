# UAT Test Plan — v1.0.0: First stable release (ROUND 3)
...
**rebuilt 2026-08-03 from `main` = `a3b60b5`**, which carries every round-2
remediation (PRs #110–#119). The rebuild was proven against the artifact rather
than against config: **10 asar markers each read present in the new binary and
absent in the round-2 one**, plus a **negative** marker — HA-04's old dropdown
label `Select replacement entity` reads 1 in the round-2 binary and **0** in this
one, which proves a removal no positive marker can. `resources/templates/` ships
9 files with no drift, the Squirrel installer is self-consistent (its `RELEASES`
sha1 and byte count both match the nupkg), and the app was confirmed to launch.
⚠ The round-2 binary is archived beside it as
`HA Visual Dashboard Maker-win32-x64-ROUND2-2026-07-31` — **do not test that one**.
**Automated baseline:** unit 1215 (95 files) · integration 215 passed / 1 failed
/ 19 skipped · e2e 285 passed / 8 failed / 2 skipped
...
`docs/testing/uat/matrices/uat_matrix_v1.0.0-r3_2026-08-03.html` is generated from
it and exists only to execute it. If the two disagree, the plan is right and the
matrix is a bug.

### ⭐ What is different about round 3

**Use the round-3 matrix**, not round 2's, and not any copy saved to your Desktop
on 31 July. What changed:

1. **Your round-2 verdicts will not reappear.** The matrix stores results under a
   key derived from the round id, and round 3 uses a new one (`v1_0_0_r3`).
2. **Every card shows a faded previous-round dot** — what round 2 recorded. A card
   that was green and is now red is a **regression**, and that is why §3.6 asks
   for a full re-run rather than only the failures.
3. **Cards carry a status badge.** `fixed — recheck` means a defect you raised is
   ...
   round 2 could not run it because something else was broken. `untested in round
2 — must be run` marks the two cards §3.1 criterion 5 will not let us leave
   blank again.

⭐ **Every High from round 2 is fixed in merged code.** HA-02 and HA-07 (#110),
FILE-04 (#111), PROPS-03 (#112), CLIP-04 (#113), HA-03 (#114 + #115) and HA-08
(#117). Nothing was accepted-with-rationale — §3.1 criterion 1 does not allow
that for a High. **Nothing further needed building before this round.**

⚠ **Two cards were WRONG in round 2 and have been corrected. This is the first
round to run them as they should have been written.**

- **HA-04 step 1** told you to use `light.does_not_exist_uat`. That id is
  unmappable **by design** — it is built to resemble nothing, and this instance
  has no `light` domain at all, so even the domain part of the similarity score
  is unavailable. The auto-mapper is in fact **correct**: measured against 725
  live entities it scores a real rename at 1.00. The card manufactured its own
  failure. Step 1 now asks for a plausible rename of an entity that really
  exists.
- **HA-08** promised that "the embedded Home Assistant render is recognisably the
  dashboard you designed". **Live Preview renders no iframe and has not since
  December 2025**; Home Assistant serves `X-Frame-Options: SAMEORIGIN`, so
  embedding is a security decision deferred past 1.0. That expectation could
  never be met. The card now sends you to a **browser** for the render — the
  observation is re-pointed, not deleted, because it is the single most valuable
  one in the round.

Both corrections are recorded in `docs/testing/uat/CARD_CORRECTIONS.md`. ⭐ The
round-2 plan was deliberately **not** edited — it is the record of what was
tested, and rewriting it would falsify the round-2 evidence.

⭐ **CLIP-02 shows a GREEN previous-round dot even though the round-2 session JSON
records a fail.** The owner re-marked it Pass on 2026-08-03: your note recorded an
Acronis alert about HAVDM writing its own `%APPDATA%` config, which is an
unsigned-binary heuristic rather than a clipboard defect, and it met none of the
High criteria. The export is never edited; the re-mark is recorded in
`docs/testing/uat/VERDICT_REMARKS.md` and applied on top. **So a CLIP-02 failure
this round is a genuine regression** — which a stale red dot would have hidden.
...
- [ ] `./tools/checks` green — verified 2026-08-03 on `a3b60b5`, REAL_EXIT=0, all
      four steps ran (lint 0 errors / 145 warnings, format clean, typecheck 0,
      unit 1215 passed / 95 files)
- [ ] Suites captured and triaged — e2e 285/8/2 on 295, integration 215/1/19.
      ⭐ Measured on PR #118 and still valid for `a3b60b5`: the only commit since
      is PR #119, which was docs-only and changed zero `src/` files. The two
      above-baseline failures (`card-context-menu:71`, `entity-browser:380`) are
      watched flakes, both exonerated by reach — the changed code cannot execute
      in either spec
- [ ] `npm run make -- --platform=win32 --arch=x64` succeeds — verified
      2026-08-03, exit 0, and the installer proven by a paired marker probe
      (10 positive + 1 negative)
- [ ] All milestone PRs merged — **#119 merged as `a3b60b5`, no open PRs**
...
reasons this round exists. ⚠ **And do not reuse the round-2 binary**, which is
archived beside it as `HA Visual Dashboard Maker-win32-x64-ROUND2-2026-07-31`
(round 1's is archived as `-ROUND1-2026-07-27`); those predate the fixes you are
here to re-test.
...
- ⚠ If your antivirus warns about HAVDM writing `config.json` in `%APPDATA%`,
  **that is not this card**. The binary is unsigned, so ransomware heuristics
  flag it; code signing is tracked separately. Judge this card only on whether
  the copy and the original are independent, and mention the antivirus prompt in
  the notes if you see it. That conflation is what made round 2's CLIP-02 a
  High, and the owner has since re-marked it Pass.
...
1. ⭐ **Without selecting anything first**, hover the right edge of a card inside
   a section until a width handle appears. Round 2 failed here: the handle did
   not exist in the DOM at all until the card was selected, and it was drawn
   transparent — so it could not be found by hovering, which is the only way a
...
- ⭐ Width and height handles are separately discoverable **by hovering an
  unselected card** — you should not have to select it first, or already know
  they are there.
...
1. In the Version Control dialog, click the repository-selection control.
   ⭐ If a repository is **already** designated from a previous session, use the
   **Change…** button — round 2 failed precisely because that state had no route
   to the picker at all, while round 1 (which had no repository yet) saw the
   button and passed. Same build, different entry state.
...
- A native folder dialog opens; only a directory can be chosen — ⭐ **whether or
  not a repository was already designated**.
...
1. ⭐ Open a dashboard, then edit one card's entity to a **plausible rename of an
   ...
   e.g. append `_old`. ⚠ Do **not** invent an id in a domain your instance does
   not have (this instance has no `light` domain). The matcher scores on domain
   and name similarity, so an id built to resemble nothing cannot map, and the
   test would measure nothing. Round 2's card made exactly that mistake.
2. Click **Remap** in the toolbar.
3. Click **Auto-map All** and read the message the dialog shows.
4. Read what the dialog reports as missing.
5. Choose a replacement entity from the dropdown for that entity.
6. Apply, then check the card on the canvas and in the YAML.
7. Look at the **History** tab of the remapping dialog.
8. ⭐ Now **disconnect from Home Assistant** (or start from a profile that has
...
- ⭐ **Auto-map All says what it did** — how many entities it mapped, or plainly
  that nothing scored high enough. **Silence is a fail:** a control that cannot
  report its own outcome is indistinguishable from a dead one, and that was the
  real defect behind round 2's "auto-map doesn't work".
- ⭐ The replacement dropdown is searchable across **every** entity by id, name or
  integration — not just a handful of suggestions — and shows which integration
  each entity belongs to. Round 2 offered only the top five, so an entity outside
  them was unreachable by any amount of typing.
...
3. ⭐ Read the **address row** at the top of Live Preview. Click **Copy address**
   (or the **Open** button) and open that address in a browser.
4. ⭐ **In the browser**, compare the real Home Assistant render to your HAVDM
   canvas. This is where the fidelity judgement happens — HAVDM does not embed
   the render.
5. Back in HAVDM, switch between views using the preview's view switcher, and
   toggle between **Edit Mode** and **Preview Mode**.
6. **Click Close to exit Live Preview.** Read the confirmation.
7. **Teardown — required:** in a browser, open Home Assistant → **Settings →
...
- ⭐ **Opened in a browser**, the real Home Assistant render is recognisably the
  dashboard you designed — this is the single most valuable observation in the
  round. Note any card that renders differently from the HAVDM canvas.
- ⭐ The address sits in **its own row**, never hidden under a card, and copies in
  one click. Round 2's failure was that it sat under the card overlay and could
  not be selected.
- ⚠ HAVDM does **not** embed the Home Assistant render, and now says so on the
  card: _"The boxes below arrange the layout; they are not the render."_ The
  boxes are the layout editor. **This is not a defect to raise** — Home Assistant
  serves `X-Frame-Options: SAMEORIGIN`, so restoring embedding is a security
  decision deferred past 1.0.
- The view switcher moves between views, and the Edit Mode / Preview Mode
  toggle switches between arranging cards and previewing.
...
| Property                                              | Cards                                            |
| ----------------------------------------------------- | ------------------------------------------------ |
| `auto_covered: true` (each names a real spec file)    | 59                                               |
| `auto_covered: false` (the card is the only evidence) | 7                                                |
| `needsHA: true` (all in group 11)                     | 9                                                |
| Creates something in Home Assistant                   | 2 — HA-08 and HA-09, both with numbered teardown |

⚠ **Correction, measured from the matrix data rather than carried forward:** the
round-1 and round-2 plans both printed **57 / 9** here, but every matrix since
round 1 has actually held **59 / 7**. The stale pair was copied forward twice
unchecked. The seven cards carrying their own weight are `SHELL-03`, `FILE-06`,
`CANVAS-04`, `CANVAS-06`, `CLIP-06`, `VIEWS-05` and `VCS-04`. ⭐ This does not
change §3.1 criterion 4's outcome — of the seven, only **VIEWS-05** failed round
2, and #118 gave it coverage — but a hand-maintained number that feeds a release
criterion is worth deriving rather than trusting.

⭐ 59 of 66 cards are `auto_covered`. That is the point of the ratio, not a
...
**Previous round:** `v1.0.0-r2` (2026-07-31), read from
`docs/testing/uat/sessions/uat_session_v1.0.0-r2_2026-07-31.json` — 66 cards,
48 pass / 13 fail / 3 skip / 2 untested as exported.

`PREV` carries **64** entries. `PROPS-07` and `YAML-05` are **omitted**, not
defaulted: they were Untested, and "untested" is not a prior _result_, so both
render grey. ⭐ **`CLIP-02` is overridden to `pass`** per
`docs/testing/uat/VERDICT_REMARKS.md` — the session JSON records `fail` and is
deliberately not edited; the owner's re-mark is layered on top when `PREV` is
built. Applying the re-marks, round 2 stands at **49 pass / 12 fail / 3 skip /
2 untested**, with **7 High / 4 Medium / 1 Low** — and every one of those Highs
is fixed in merged code.
