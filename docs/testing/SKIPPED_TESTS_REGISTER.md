...
- Primary causes:
  ...
  - Bulk multi-select property edit undo granularity (known product/store behavior gap)
...
| `tests/integration/bulk-operations.spec.ts` | `applies bulk property edit to selected cards and preserves undo granularity` (`test.fixme`) | Skipped | Feb 17, 2026 | Bulk multi-select property edit currently records more than one history entry; a single undo does not deterministically restore all edited cards in one step. | Revisit after store update path is changed to a single immutable batched commit for multi-select property edits. |
