...
**Status:** Draft — revision 4, 2026-09-07 · answers `304aeca` (P6 remainder and P15 at SEV 1; P16 and P17 at SEV 3), owner-ruled 2026-09-07 as one final, shrinking revision; awaiting the last scoped follow-up. Earlier revisions: §10.
...
**Parent objective:** the owner's goal, stated 2026-09-06 — "we are spending more time reviewing and fixing than creating production code"; non-blocking, low-impact defects are to be recorded and triaged for a later cleanup sweep, with nothing a reviewer may REPORT reduced and nothing a SEV 1 means weakened. · **Cheapest acceptable outcome:** rulings 1–6 landed (component-scoped blocking, reachability, the Owner Summary Table, the derived verdict, the two non-repair disposition states, owner decides per Ref); rulings 7–9 could fall to a follow-up story without losing the round-count effect. · **Cost stop-rule:** if plan review plus edits plus ratification review exceed three review rounds in total, or one working week of wall-clock, work halts and re-asks the owner (mirrors §3.3's wall-clock trigger). ⚠ **Re-authorised by the owner on 2026-09-06 after the second review round:** four rounds in total — the follow-up on this revision, then the ratification review of the edits — under a hard cap: if the follow-up on this revision returns anything other than `CLEAR` or `CLEAR-WITH-FINDINGS`, work parks and returns to the owner; if the ratification review blocks, the author returns to the owner rather than repairing. ⚠ **Re-authorised a second and LAST time, 2026-09-07:** one shrinking revision and one follow-up; anything other than `CLEAR` / `CLEAR-WITH-FINDINGS` parks B14 and rescopes it (rulings 1–6 landed directly under one ratification review); no further re-authorisation. Basis: Round 3 of the dispositions ledger.
...
| All seven SEV-2s sit in one seam — **JUDGEMENT, hand trace**                                                                                       | Seam trace in §5, traced by hand row by row from each finding's headline and problem statement; not the output of any command (follow-up P14)                                                                                                                                                                                                                                                 |
| Every one of the eight review files carries a "Class swept" statement                                                                              | `grep -c 'Class swept' docs/reviews/plan-consistency-c3-parser-implementation-review*.md` — a non-zero count for each of the eight files                                                                                                                                                                                                                                                      |
...
Every row that names a command is MEASURED and was re-derived from the review files for revision 2 — revision 1 carried 13 and 20, copied from an earlier session's note rather than re-derived, and the plan review's P4 caught it. The seam-membership row is a JUDGEMENT — a hand trace — and is labelled as one here and in §5 (follow-up P14).
...
| 2   | To block, a reviewer must also show the condition that reaches the defect — an input, a path through the code, a state, an output, or a property of the built artifact itself — and that it exists today. A made-up condition never blocks an ordinary component; only a safety gate, an artifact whose whole job is catching future mistakes, may block on a future edit, and only after the reviewer argues why that edit is plausible. | Template §4; OA §4 row |
...
**Cost.** Every SEV 2 and SEV 3 finding now arrives on your desk as a short brief you rule on by reference; SEV 4 is recorded and needs no ruling. A repeat of the same seam adds one continue / declare-residual / park choice. This plan does not predict how many rulings a pull request will take — that depends on your choices; §5 shows only the floor and the ceiling in rounds. Deferral moves work to a cleanup sweep rather than deleting it, except where you declare a seam a residual — for PR #154 the whole seam guarded inputs nobody has written, so declaring it would have removed the work.
...
- The STRAT-D7 scoped follow-up on revision 2: `docs/reviews/b14-severity-rulings-codex-plan-review-followup.md` (`cd1d412`, `BLOCKED-ON` three sections — P2, P3 and P6 PARTIALLY RESOLVED with SEV 1 remainders; P7 and P8 PARTIALLY RESOLVED with SEV 2 remainders; new P13 and P14 at SEV 3). Under this plan's own same-seam rule the author put a continue / declare-residual / park choice to the owner per seam, with pros and cons and a recommendation, and a fix-now / defer / accept-residual brief per non-blocking finding; on 2026-09-06 the owner ruled continue on all three seams and fix-now on P7, P8, P13 and P14, authorised the two Issue #156 corrections, and re-authorised the cost stop-rule to four rounds under the cap in the header. Revision 3 answers them; Round 2 of the dispositions ledger records the rulings.
...
| Sweep: six-token inventory + labelled read of the live surfaces | §3.6         | enumeration only; 3 live surfaces edited, the rest untouched           |
...
- **The owner's fix-or-defer decision (SEV-CAL-2 ruling 6).** Before
  ...
  committed. A repair required for a SEV 1 may overlap it **only to the
  extent the overlapping change is inseparable from resolving the SEV 1**: a
  lower-severity correction that could be left out of the SEV 1 repair is
  separable, and waits for the owner's ruling whatever the repair is called.
  Where the overlap is inseparable, the lower-severity Ref stays OPEN until
  the owner records how the incidental closure is dispositioned. SEV 4
  findings are recorded in the table, need no brief, and the owner may elect
  a fix. **DEFERRED and ACCEPTED-RESIDUAL are
  ...
  label never changes the substance of a diff. A REPAIR is a change to the
  artifact under review, or to a safeguard for it — a test, a `KNOWN-OPEN:`
  pin, a governed residual — made to resolve or mitigate a finding; it is a
  repair whatever its disposition row calls it, and it receives its
  follow-up. Review reports and other decision or evidence records that
  change no reviewed target and no safeguard — the disposition table, the
  board item, the owner's brief, the reviewer's committed review — are not
  repairs.** A SEV 1 leaves OPEN only as RESOLVED, or by an
...
### 3.6 The sweep — the six-token inventory, and the labelled read of the live surfaces
...
| This branch's own new files — never listed statically here                |     + | Change artifacts, excluded from the inventory; named only in the edits-commit record, from `comm -13` of Enumeration 1 run on `main` against Enumeration 1 run on the branch                                                                                                                                                                                                                                     |
...
`tests/support/authorLedger.ts:68` names the governed set — `docs/governance/**`, `docs/templates/**`, `ai_rules.md`, `CLAUDE.md`. Every commit that changes a governed file moves the fingerprint, so `docs/reviews/self-pass-gate-author-ledger.md` is regenerated after each such commit with a dated addendum; how many times is read from the ledger's addenda, never stated here. ⚠ `git add` the governed files BEFORE computing the fingerprint — the certificate hashes the index entry as well as the working tree (`tests/support/authorLedger.ts:290-340`; `7d3969f`). The commissioned rows are not touched.
...
```bash
# Source population: one heading per finding. Dispositions: one plan row per finding.
# Neither side may hold a duplicate; the two Ref sets must be identical; there must be exactly 21.
# The block's exit status is the check: 0 only when every line passes.
src=$(grep -hoE '^### P[0-9]+ — SEV-[1-4]' docs/reviews/plan-consistency-c3-parser-implementation-review*.md | grep -oE 'P[0-9]+')
dis=$(grep -oE '^\| P(1[2-9]|2[0-9]|3[0-2]) ' docs/testing/PLAN_CONSISTENCY_C3_PARSER_PLAN.md | grep -oE 'P[0-9]+')
ok=1
[ -z "$(echo "$src" | sort | uniq -d)" ] || { echo "FAIL: duplicate source heading(s): $(echo "$src" | sort | uniq -d | tr '\n' ' ')"; ok=0; }
[ -z "$(echo "$dis" | sort | uniq -d)" ] || { echo "FAIL: duplicate disposition row(s): $(echo "$dis" | sort | uniq -d | tr '\n' ' ')"; ok=0; }
diff <(echo "$src" | sort -u) <(echo "$dis" | sort -u) >/dev/null || { echo "FAIL: Ref sets differ:"; diff <(echo "$src" | sort -u) <(echo "$dis" | sort -u); ok=0; }
[ "$(echo "$src" | sort -u | wc -l)" -eq 21 ] || { echo "FAIL: $(echo "$src" | sort -u | wc -l) unique findings, expected 21"; ok=0; }
[ "$ok" = 1 ] && echo "replay population ok: 21 findings, one disposition each, sets identical"
[ "$ok" = 1 ]
```

(The heading key is unique per finding. Revision 1 regenerated from Owner Summary rows and emitted 34 lines, because claim-ledger rows also begin `| P<n>` — plan review P7. Revision 2 printed two bare counts, which still pass when a duplicate on one side is offset by an omission — follow-up P7; the block above fails on a duplicate on either side, on any difference between the two Ref sets and on a count other than 21, and was proved to fail on each constructed case in Round 2 of the dispositions ledger. The eighth round's Owner Summary Table has a different column layout and is read for its recommendation only.)
...
**Rounds — the floor and the ceiling, MEASURED from the replay; this plan makes no numeric projection of owner rulings.** The floor is one round: the owner defers or accepts every non-blocking finding at round 1, so no repair and no follow-up occur. The ceiling is the real history plus the follow-up it missed: nine rounds, if the owner elects Fix now on every finding, because every repair still earns its STRAT-D7 follow-up and each recurrence of a seam adds a continue / declare-residual / park choice. Between the two, the owner decides per finding. The saving is real only where a seam guarded inputs the governed plan has never contained (§1.2); for a reachable seam the same decisions would ship a pinned gap, and that is the owner's call each time.
...
| AC-2 | The six-token inventory (§3.6 Enumeration 1) is complete and partitioned on the edits commit, and the named live surfaces (§3.6 Enumeration 2) received a labelled behavioural read; the `prompts/` pass reported separately as session-local. ⚠ NOT claimed: that every tracked restatement is swept — §3.6 records that class as undecidable by search (follow-up P8) | §3.6's token inventory on the edits commit equals `main`'s 100 (partition 87 / 3 / 6 / 1 / 3 / 0 unchanged) plus only this branch's own new files, each named; the behavioural claim is limited to the surfaces §3.6 Enumeration 2 names; the `prompts/` figure is reported as session-local                                       |
...
| AC-4 | The STRAT-D7 trigger text is byte-identical                                                                                                                                                                                                                                                                                                                             | The command block below the table exits 0 with empty output for the blockquote and for each named bullet                                                                                                                                                                                                                           |
| AC-5 | The replay (§5) is present with a regenerating check that fails on the defects it guards                                                                                                                                                                                                                                                                                | Reviewer re-runs the §5 block: it prints `replay population ok: 21 findings, one disposition each, sets identical` and exits 0; a duplicate heading, a duplicate row, a Ref-set difference or a count other than 21 prints `FAIL` and exits 1 (each proved on a constructed known-bad input in Round 2 of the dispositions ledger) |
...
**Accepted operational cost, stated rather than buried.** One ledger regeneration per governed commit (§3.7). Every SEV 2 and SEV 3 finding on every future PR reaches the owner as a brief (§1.4).
...
4. **§3.3 protected span** — Does either inserted bullet — including revision 3's definition of a REPAIR by its target (the artifact under review or a safeguard for it) with review and decision records outside it — change the MEANING of the unchanged STRAT-D7 bullets, even though their bytes are unchanged? In particular: does the definition exempt any change to a reviewed target or safeguard that STRAT-D7 would follow up today?
5. **§5** — Is the seam trace defensible? Name any finding you would move between seams A, B and C.
6. **§3.2** — Does the implementation reading of SEV 1 (a contract violated under a condition that exists today — an input, an execution path, a state, an output, or a property of the artifact itself — or under a future condition the reviewer has argued plausible) leave a class of real defect ungradeable, or let a merely constructible condition block? Give the construction. (Round 1 answered the narrower "input" wording with the missing-manifest construction; the owner broadened the rule on 2026-09-06; the follow-up found no loophole in the broadened technical text and faulted only the two restatements this revision rewords.)

## 9. Open questions for the owner

None open. P6's broadening of ruling 8 — "input" becomes "an input, an execution path, a state, an output, or a property of the artifact itself" — was put to the owner by the plan review and ruled ACCEPTED on 2026-09-06. The eighth-round STRAT-D7 miss on PR #154 (P4) was ruled RECORD ONLY, not re-opened. The one default the agent proposed (SEV 3 defaults to DEFERRED) was declined on 2026-09-06 and is not in this plan; it is available as a later ruling if the per-finding brief becomes a burden. On 2026-09-06, after the second review round, the owner ruled continue on the P2, P3 and P6 seams, fix-now on P7, P8, P13 and P14, authorised the two corrections to Issue #156's acceptance items, and re-authorised the cost stop-rule to four rounds under the cap recorded in the header; each ruling was taken on a pros-and-cons brief and is recorded in Round 2 of `docs/reviews/b14-severity-rulings-repair-dispositions.md`. Third-round ruling (2026-09-07, `304aeca`): one final shrinking revision under a terminal cap — Round 3 of the same ledger.
...
| 2026-09-06 | 3   | Answers the STRAT-D7 follow-up (`cd1d412`), every finding owner-ruled as recommended after a per-seam continue / declare-residual / park brief: overlap confined to the portion inseparable from the SEV 1 repair (P2); a REPAIR defined by its target, with review and decision records outside it (P3); the two stale "input" restatements in §1.3 and §8 reworded to the ruled condition vocabulary (P6); the replay check made a set-equality block that fails on duplicates, omissions and count (P7); AC-2 and the §2.1/§3.6 headings cut to the decidable result (P8); Issue #156 acceptance items 2 and 5 corrected (P8, P13); the seam-membership row split out and labelled JUDGEMENT (P14); cost stop-rule re-authorised to four rounds with a hard cap | Claude Fable 5.1 |
| 2026-09-07 | 4   | Answers `304aeca` by shrinking: P6 row qualified; scenarios and ruling counts removed (P15); counts replaced by the commands that read them (P16, P17); terminal cap. Detail: dispositions Round 3                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Claude Fable 5.1 |
