...
| Every §4 fact                | Measured with the command printed beside it. ⚠ **Corrected 2026-09-09:** this row originally read "measured on `691c8d1` on 2026-09-08", which four repair rounds falsified — F5 and F7 carry facts **re-measured on 2026-09-09** (HACS metadata filling only `versions`; absolute-URL and query-string retention; the docblock/implementation disagreement; `cardAvailability.ts:40` and its two consumers). The base commit is unchanged; the dates are not. ⚠ **Extended 2026-09-10 (finding P7):** §0 and the §4 preamble carried the same unqualified 2026-09-08 universal and now carry this qualification too. The F5/F7 enumeration is **measured, not asserted** — the deciding command is printed **below this table**, and its output is the account. **This round then edited F4 itself** — its _summary wording_ only (finding P8); no F4 fact was re-measured because none moved, and the F4 population was re-confirmed unchanged on 2026-09-10 (seven files match the names; the five call sites are still `App.tsx:426`, `:748`, `:2481`, `:2580` and `HADashboardIframe.tsx:197`) |
