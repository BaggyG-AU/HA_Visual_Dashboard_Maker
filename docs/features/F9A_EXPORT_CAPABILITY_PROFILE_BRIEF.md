...
list. Every one was re-measured on this branch's base (`main` = `691c8d1`) with
the command printed beside it — on **2026-09-08**, except for the facts a later
repair round re-measured on **2026-09-09**. **§11 names which, and carries the
command that decides it**; the list is deliberately not repeated here, because a
second copy of it would drift at the next repair. The base commit is unchanged;
only the dates are. Nothing in §4 was inherited from the withdrawn spec without
...
Every fact below was measured on `main` = `691c8d1` — on **2026-09-08**, except
for the facts a later repair round re-measured on **2026-09-09**; **§11 names
which, and carries the command that decides it**. Line numbers
...
the enclosing function and following where its output actually goes:**

| #   | `path:line`                                | Enclosing function                     | What the call produces, and where it goes                                                                                                                                                                                                                                                        |
| --- | ------------------------------------------ | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
⭐ **The distinction the spec needs.** Sites 1, 2, 3 and 5 decide the
**content** itself — sites 1, 3 and 5 hand it to **Home Assistant**, and site 2
writes it to a **file on disk**, where Home Assistant is not involved at all.
Site 4 produces no content; it decides only what the user is _told_ before
...
| Every §4 fact                | Measured with the command printed beside it. ⚠ **Corrected 2026-09-09:** this row originally read "measured on `691c8d1` on 2026-09-08", which four repair rounds falsified — F5 and F7 carry facts **re-measured on 2026-09-09** (HACS metadata filling only `versions`; absolute-URL and query-string retention; the docblock/implementation disagreement; `cardAvailability.ts:40` and its two consumers). The base commit is unchanged; the dates are not. ⚠ **Extended 2026-09-10 (finding P7):** §0 and the §4 preamble carried the same unqualified 2026-09-08 universal and now carry this qualification too. The F5/F7 enumeration is **measured, not asserted** — the deciding command is printed **below this table** and reports **F5 and F7 differ and nothing else**. **This round then edited F4 itself** — its _summary wording_ only (finding P8); no F4 fact was re-measured because none moved, and the F4 population was re-confirmed unchanged on 2026-09-10 (seven files match the names; the five call sites are still `App.tsx:426`, `:748`, `:2481`, `:2580` and `HADashboardIframe.tsx:197`) |
| Every drawer ID and board ID | Read back from a tool result, never typed from memory                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Not established here         | §F3 and §F4 are **hand traces** over source, labelled as such; they are not evidence that any test currently exercises these paths. ⚠ **Corrected 2026-09-09:** this row originally read "No runtime behaviour was executed", which is no longer true — the author executed a Node probe against the real capability modules on 2026-09-09 to verify review findings. **No Electron, e2e, integration or live-HA run was performed, and none is needed for a docs-only branch**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |

**The command behind the "Every §4 fact" row.** It splits §4 into its `### F`
subsections at two **immutable** heads — `0bfeb6c` (the original brief) and
`de0736c` (the head the 2026-09-10 repair started from) — and compares them.
⚠ Pinned to named commits deliberately: run against a moving `HEAD` it would go
false at the next repair to any §4 subsection
(`drawer_practice_review_41e379f2e616eba581924b07`).

```bash
B=docs/features/F9A_EXPORT_CAPABILITY_PROFILE_BRIEF.md
for r in 0bfeb6c de0736c; do
  mkdir -p "/tmp/$r" && git show "$r:$B" |
    awk -v d="/tmp/$r" '/^## 5\./{f=0} /^### F[0-9]/{f=1;n=$2} f{print > (d"/"n)}'
done
