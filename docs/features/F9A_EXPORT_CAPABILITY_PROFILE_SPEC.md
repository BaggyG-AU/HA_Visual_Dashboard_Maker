...
| Codex spec review, `docs/reviews/f9a-spec-codex-followup3.md`, P2/P8 partial + new P10 (owner ruling, second same-seam occurrence: continue with an isolated test hook for P8, moot-by-construction for P10, declared residual for P2) | §7 D-5 (residual), "Establishing a KNOWN source dashboard" (isolated hook); §8 AC-17; §9 Leg 13 (KNOWN-OPEN) | #159    | Covered |
...
| `tests/e2e/live-preview-deploy.spec.ts`             | **(P1 repair)** Extend `stubLivePreviewIpc` to record the `config` argument each stubbed IPC channel receives (currently discarded); add Legs 7–10 (§9) covering sites 3 and 4 using the existing offline harness. Site 5's wiring is a required review-time check (§9), not a new e2e leg — see §9's note on why. **(Round 4.)** New `stubDashboardBrowser(ctx)` helper also stubs `ha:ws:connect` (repair for review Ref P8 construction B). New Leg 13 (KNOWN-OPEN, §9) pins the D-5 residual accepted for review Ref P2.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
**Pinned by a `KNOWN-OPEN:` leg (§9, Leg 13)**, per this project's own
convention for a boundary written in prose that nobody re-reads
(`drawer_practice_claims_588b2f2df00141d8f19f9433`; the same pattern D-6
already uses for the boot window). A future repair that closes this gap
must update Leg 13 (it will start failing) and this section together.
...
17. **(Round 4, owner-accepted residual for review Ref P2.)** An
    already-shown live-preview deploy confirmation (`HADashboardIframe`'s
    first, generic `Modal.confirm`) is **not** revoked by a layout write
    that begins after the dialog opens, from a drag/resize gesture already
    in progress at the moment the "Deploy to Production" button was
    clicked — pinned by a `KNOWN-OPEN:` leg (§9, Leg 13), not demonstrated
    safe. This is an accepted limitation of D-5's button-disable mechanism,
    not a defect this specification claims to have closed.
...
(connected: boolean) => setIsConnected(connected)` — **and add a new,
  ...
  `haConnectionService` and `__testThemeApi` is unaffected. This is also a
  narrower change than round 3's: it touches only `haConnectionService`,
  not React's `isConnected` state, because nothing in Legs 9–12's own flow
  needs React `isConnected` true — the toolbar "Download" button
  (`App.tsx:3290`) is unconditionally enabled, and `handleOpenDashboardBrowser`
  itself no longer gates on `isConnected` either (`App.tsx:2257-2266`); only
  `DashboardBrowser.loadDashboards`'s own service check needed satisfying.
- A new DSL step, `establishKnownSourceDashboard(ctx)`: calls
  ...
  (round-4 addition — this step's own prerequisite, not the shared connect
  step), then clicks the toolbar "Download" button
  (`data-testid="toolbar-download"`, `App.tsx:3290`), waits for the
  Dashboard Browser dialog, clicks "Download" on the always-present
  ...
  `stubLivePreviewIpc`'s existing four channels are needed. **Round 4:**
  Legs 9–12 no longer call the shared `appDSL.setConnected(true)` step at
  all — this one step now establishes everything the Dashboard Browser
  needs, via the isolated hook above, not the shared one.
...
9. **Leg 9 (RED-BEFORE-GREEN — site 4, words agree with the freshly-written
   ...
   Call `establishKnownSourceDashboard(ctx)` (not "create new") — **round
   4:** this one step configures `haConnectionService` via the new isolated
   hook (repair for review Ref P8) and drives the download; no separate
   "connect" step, shared or otherwise, is needed. Enter live preview (site 3 writes stripped bytes, using
...
11. **Leg 11 (RED-BEFORE-GREEN — site 4, a failed confirm-time write must
    ...
    instead. **Red on base** — base has no confirm-time write and no
...
12. **Leg 12 (site 5, the Deploy button — round 3 repair for review Ref P2,
    ...
    have settled. **Red on base** — base has no `pendingLayoutWrites`
    counter and the button's `disabled` condition never reflects a pending
...
13. **Leg 13 — KNOWN-OPEN leg (D-5's declared residual, round 4, owner
    ruling on review Ref P2, `docs/reviews/f9a-spec-codex-followup3.md`,
    confirmed 2026-09-14).** Pins the CURRENT, accepted behaviour rather
    than demonstrating safety, per
    `drawer_practice_claims_588b2f2df00141d8f19f9433`'s guidance ("pin a
    KNOWN-OPEN limitation with a test that asserts the CURRENT, passing
    behaviour") — the same convention Leg 6 already uses for D-6's boot
    window. Same known-source setup as Leg 9. Begin a drag on a card (mouse
    down, move) but do **not** release it yet — `pendingLayoutWrites` is
    still zero, because `handleLayoutChange` only runs at drag **stop**.
    Click "Deploy to Production" and accept the first, generic confirmation
    — the button is enabled (count zero) and `onOk: onDeploy` is bound now,
    before the in-progress drag's write is ever counted. Using the same
    releasable-gate technique as Leg 12 (a controllable promise gating
    `haWsIsConnected` inside the stubbed IPC), hold the drag's own
    `handleLayoutChange` write pending, and release it only **after**
    site 4's confirm-time re-derive-and-rewrite (D-5 step 3) has completed.
    Assert: the temp dashboard's final recorded content is the drag's
    (unstripped, stale) bytes, not site 4's freshly-derived ones — i.e., the
    known gap **is** currently reproducible, not merely theorised.
    **Passing on base and after the round-3/round-4 repairs alike** — this
    leg does not turn red when the repair lands, because the repair does not
    close this window; it is named `KNOWN-OPEN:` specifically so that a
    future change which **does** close it breaks this assertion, forcing
    the fix and D-5's residual note to be updated together. Proves nothing
    new about AC-1/AC-2/AC-5 (Leg 9 already does); exists solely to keep the
    accepted residual honest and visible.
...
4. The review's scoped follow-up (Operating Agreement §3.4) confirms the
   ...
   **The third follow-up (`docs/reviews/f9a-spec-codex-followup3.md`) then
   ...
   site 5's. Rev 4 is that ruling. Whether this fourth pass fully closes
   what remains live in this seam (P8's narrower repair) is for the
...
has now fired TWICE on one mechanism (site 4's confirm-time flow and its
Dashboard-Browser test setup) across four external review rounds.** First
firing (Rev 3): round 1 fixed the referential-stability race but did not
sweep its own new mechanism's failure path, the pending-writer race, or the
route it actually reaches (three gaps, closed in Rev 2); Rev 2's OWN
pending-writer fix then proved wrong under out-of-order completion, closed
in Rev 3 with a design change (disable, don't await). Second firing (this
Rev 4): Rev 3's own design left the already-issued-confirmation window open
(P2, now declared residual) and Rev 3's own test-only fix was both
incomplete and over-reaching (P8/P10, now closed with a narrower, isolated
hook). **If the next scoped follow-up finds a further live defect in this
same seam, that is a THIRD occurrence of this trigger.** The author will
name it as such explicitly — not as a fifth instance patch, and not framed
the same as the first two recurrence briefs — and state plainly whether, in
the author's own judgement, the mechanism itself (not merely its latest
instance) is what needs to change, since two design-level interventions
having already been made to the same site is itself evidence the seam may
be structurally harder to close than a third bounded fix would suggest.
