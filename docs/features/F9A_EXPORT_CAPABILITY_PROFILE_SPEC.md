...
carrying style: 'color: red;'> }`, matching the card content Legs 7–12
  already use).
- **Prerequisite fixed, round 3 (repair for review Ref P8,
  `docs/reviews/f9a-spec-codex-followup2.md`, confirmed 2026-09-14): the
  existing preview stub does not configure the service the Dashboard
  Browser itself checks.** `appDSL.setConnected(true)` (used throughout
  Legs 1–12) sets only React's `isConnected` state via the
  `__testThemeApi` backdoor — confirmed by reading
  `App.tsx:569-571` directly. `DashboardBrowser.loadDashboards`
  (`DashboardBrowser.tsx:97-100`) instead checks the separate
  `haConnectionService.isConnected()` (`haConnectionService.ts:73-75`,
  `this.config !== null`), which `setConnected(true)` never touches —
  independently reproduced 2026-09-14 by executing the actual
  `loadDashboards` body: with only React state set, it records
  `'Not connected to Home Assistant'` and never calls `haWsListDashboards`
  at all. **Fix:** `__testThemeApi.setConnected` (`App.tsx:569-571`, the
  same test-only, `isTestEnv()`-gated backdoor, widened rather than
  replaced) also calls `haConnectionService.setConfig({ url: 'http://example.invalid', token: 'test-token' })`
  when connecting and `haConnectionService.disconnect()` when
  disconnecting — so the one existing "connect" step in a test now
  configures both pieces of state the production code actually checks,
...
9. **Leg 9 (RED-BEFORE-GREEN — site 4, words agree with the freshly-written
   ...
   Connect (via the widened `setConnected(true)`, so `haConnectionService`
   is configured too), then `establishKnownSourceDashboard(ctx)` (not
   "create new"). Enter live preview (site 3 writes stripped bytes, using
...
4. The review's scoped follow-up (Operating Agreement §3.4) confirms the
   ...
   Whether this third pass fully closes the seam is for the **next** scoped
   follow-up to confirm — item 4 remains **pending** until that round
   returns with no further live finding in this seam.
...
⚠ **Three follow-up rounds on the same seam (site 4's confirm-time
mechanism) already triggered this section's own recorded owner decision
once** (`OPERATING_AGREEMENT.md` §3.4's same-seam rule): round 1 fixed the
referential-stability race but did not sweep its own new mechanism's
failure path, the pending-writer race, or the route it actually reaches
(three gaps, closed in Rev 2); Rev 2's OWN pending-writer fix then proved
wrong under out-of-order completion (closed in Rev 3, with a design change
— disable, don't await). **If the next scoped follow-up finds a further
live defect in this same seam, that is not a fourth instance patch — it is
the second time this trigger has fired, which the author brings to the
owner not as another continue/residual/park brief alone, but flagged
explicitly as a recurrence, so the owner can judge whether the seam's
design is the right one at all,** independently of whether any single
instance is individually fixable.
