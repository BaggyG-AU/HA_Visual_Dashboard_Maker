...
**Status:** Draft — rev 4: Codex rounds 1, 2 and 3 applied, **both open questions answered by the owner** (§10, §11). No open questions remain; awaiting owner sign-off (merge of PR #132)
**Author:** Claude Opus
**Reviewer:** Codex — on-branch review
`docs/reviews/f5-spec-insertion-contract-codex-review.md`; round 1 `76cf6d4`,
round 2 `45bcbad`, round 3 `d853a74` — all three CHANGES-REQUIRED, all three
...
Three legs mix red and green base assertions and are marked 🟠 SPLIT: **L8**
(marker red / landing green), **L9** (marker red / debug-state and landing
green), **L15** (sibling-insertion red / stack-unchanged green). The table names
which half is which so a green result is never read as proof of the wrong half.
Per `[STATE]` discriminator lesson (m), a red leg where one assertion passes on
base is stronger, not weaker — it proves which behaviour is being measured.
...
eventual usefulness — the controlling evidence. Any multi-assertion leg added
later must be audited the same way before it is given a colour.
