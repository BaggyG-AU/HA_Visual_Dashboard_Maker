...
The board is the working home for these contracts and close-outs. The story criteria carry the approved intent; they are not a claim that the product work is complete. Fable's check of the added wording is pending. Each brief/specification must still settle its open details and its own appropriate DoD before implementation. F9a's source and treatment of unknown layout-card information, for example, remain the Sonnet specification's work.

## Supporting change prepared

The [practice-index proposal](2026-09-11-practice-index-reorganisation-proposal.md) includes an actual before/after bundle and a history draft. Codex checked text preservation, rule identifiers and the active instructions at the history boundary. Fable must review it before the live index changes. The proposed index contains about 35% fewer characters; this is not a measured token saving.
...
MemPalace refused the previous checkpoint because another session held the writer lease. This session uses the staged filing for a write-enabled session rather than retrying or bypassing that lease. The approval is already visible in the board records and this committed adoption record; memory publication and its short live-state pointer remain to be completed.

Next: Fable checks the board contract additions and the proposed index relocation, files the approval if write-enabled, and reports any material disagreement together. Product work then follows the approved roles and remaining readiness requirements. Fable does not write the Sonnet specification itself. No additional owner approval of the adopted trial is requested.

## MemPalace filing candidate
