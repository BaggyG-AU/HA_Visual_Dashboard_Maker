...
test.describe('Color Picker - PropertiesPanel Integration', () => {
  test.skip(
    'visual regression and accessibility in scrollable PropertiesPanel (skipped: Electron focus inactive in PW)',
    { tag: '@visual' },
    async ({ page }, testInfo) => {
      void page;
      ...
      try {
        ...
        await colorPicker.expectPopoverWithinContainer(
          'properties-panel',
        ...
        await colorPicker.tabUntilFocused(
          window.getByTestId('button-card-color-input'),
          2,
          testInfo,
        );
        await colorPicker.tabUntilFocused(
