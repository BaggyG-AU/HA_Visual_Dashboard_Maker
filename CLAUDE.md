...
> **If the tools are not present, first work out which case you are in.**
>
> - **You are the maintainer and the config exists locally** — MCP simply
>   hasn't loaded. Reload the VS Code window (`Ctrl+Shift+P` → _Developer:
>   Reload Window_) and approve the project MCP server. Config: `.mcp.json` +
>   `mempalace.yaml` at repo root, **both gitignored**.
> - **You have no `.mcp.json` / `mempalace.yaml` at all** — you are working
>   without this project's memory store, which is the normal state for anyone
>   who is not the maintainer, since those files are gitignored and the palace
>   is machine-local. **This whole section does not apply to you.** Skip it,
>   say once in your first response that MemPalace is unavailable, and carry
>   on — it is not a blocker and there is nothing to install.
>   [`ai_rules.md`](ai_rules.md) §11's fallback path governs: use local memory
>   files, and surface drawer-candidate notes in the PR body for a
>   MemPalace-enabled agent to file later.
...
governs a delay of seconds. If the tools are absent because this checkout has
no MemPalace at all — the ordinary case for anyone who is not the maintainer —
the sequence below is **not** owed, has not lapsed, and must not be retried,
worked around, or reported as done. Say so once and proceed on
[`ai_rules.md`](ai_rules.md) §11's fallback path.
